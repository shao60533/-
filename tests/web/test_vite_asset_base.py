...
def test_preload_helper_chunk_exists():
    ...
    build was never run (deploys ship the checked-in dist). See
    ``stock_trading_system/web/frontend/README`` if any.
    """
...
def test_no_bare_assets_url_in_built_chunks():
    ...
    for js in DIST_DIR.glob("*.js"):
        ...
        # Pre-filter: cheaper than a regex on every chunk.
        if '"/assets/' not in text:
            continue
        # Pull a short surrounding context so debugging is fast.
