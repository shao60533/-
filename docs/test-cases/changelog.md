...
|------|------|------|--------|---------|---------|
| 2026-05-13 | [hardening-iteration-v1.md](hardening-iteration-v1.md) | v1.0 | 151 | **硬化迭代 v1 测试用例**（配套 [design/hardening-iteration-v1.md](../design/hardening-iteration-v1.md)）：覆盖 9 Critical + 16 High。按 5 Phase 分组——**P0 安全底座 43**（CSRF 接入 12 + 漏装 @admin_required 6 + alerts/portfolio owner 强制 12 + 限流 8 + 统一错误处理 5）；**P1 多租户契约 39**（Telegram bot 白名单 8 + 删 legacy TaskScheduler 跨租户 3 + `_user_id() raise` 6 + cross_user_access 矩阵从 3 → 14 共 14 + invariants 5 不变式 + `created_by` fallback 3）；**P2 数据可信度 33**（LocalCache ttl + 未知 category 拒绝 10 + Pickle→JSON 3 + Qwen prompt themes 外置 3 + datetime tz 统一 4 + Paper Trade Decimal 8 + Polygon 锁+Quote 抽象 5）；**P3 业务版本收敛 28**（screener v1/v2 → v3 收口 5 + backtest.py 退役口径统一 6 + DataManager→DataRouter 收口 4 + 迁移 runner 6 + analysis_history schema 统一+task_events.seq DB 化 5 + PortfolioDatabase WAL 2）；**P4 Blueprint 拆分 8**（route parity 3 + service 函数单测 3 + middleware 2）；+ **E2E 安全端到端 6**（未登录重定向 / CSRF 攻击 / IDOR 端到端 / role gate 双层 / 暴力破解 429 / Telegram 全流程）+ **性能与回归 4**（v3 缓存命中加速 / CSRF 接入 p95 / 限流 p95 / 迁移耗时）。**测试矩阵核心增量**：`cross_user_access.py` 从 3 条扩到 **14 条**（覆盖 alerts.remove / alerts.history / alerts.list / portfolio.delete / portfolio.update_cost / portfolio.snapshot / portfolio.history / analysis.delete / bookmark.delete / paper.read_other / paper.entry_other / settings.write / scheduler.start / tasks.cancel_other）；`invariants.py` 加 **5** 条（alert_history.user_id / user_analysis_advice.analysis_id FK / task_events.user_id INTEGER / daily_snapshots.user_id / tasks.created_by）。**TC 命名前缀**：`TC-HD-C{1..5}-N`（P0）/ `TC-HD-T{1..6}-N`（P1）/ `TC-HD-D{1..7}-N`（P2）/ `TC-HD-V{1..6}-N`（P3）/ `TC-HD-W{1..3}-N`（P4）/ `TC-HD-E{1..6}`（E2E）/ `TC-HD-P{1..4}`（性能）。**严格不动**前端 React Island 用例（不在本期覆盖）/ OAuth provider 主体测试（已通过审查）/ Paper Trade 业务逻辑全套（仅 P2.6 Decimal 范围）/ analyzer.py god object 用例（独立迭代）。**验证命令**：`pytest tests/ -x --tb=short` + `python -m stock_trading_system.validation.{run_all,invariants,cross_user_access,sign_off}` 每个 Phase 完成时跑，任何阶段 fail 阻断合并。 | [hardening-iteration-v1.md](../design/hardening-iteration-v1.md) |
