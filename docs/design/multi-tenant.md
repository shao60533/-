...
| 版本 | 日期 | 变更 |
|---|---|---|
| v1.1 — addendum | 2026-05-13 | **回归补齐说明**：2026-05-13 全仓 4 路审查发现本方案 §5.1（`flask-wtf>=1.2` 已写入 requirements）+ §5.5（`@admin_required` 已在 `auth/decorators.py:22` 实现）的多项约束**在 `web/app.py` 实施时未真正接入**，形成回归。具体：(a) grep `CSRFProtect\|flask_wtf` 在 `web/app.py` **0 命中** → 全应用零 CSRF 防护；(b) `@admin_required` 漏装在 `/api/settings`（[web/app.py:2654](../../stock_trading_system/web/app.py#L2654)，导致 role=user 可改全局 API key）+ `/api/scheduler/start\|stop`（[web/app.py:2630](../../stock_trading_system/web/app.py#L2630)，可拉起 legacy 跨租户调度器）；(c) `p0a_data_partition.py` 给 `alert_history` 添加的 `user_id` 列 [database.py:624 save_alert_trigger](../../stock_trading_system/portfolio/database.py#L624) 写入端从未传值，列恒为 NULL，导致 `/api/alerts/history` 跨租户读；(d) `PortfolioManager._user_id()` [manager.py:45](../../stock_trading_system/portfolio/manager.py#L45) 拿不到时 fallback `None`，DB 层 `user_id=None` 走"无过滤"分支 = 全表（多租户契约从约束降为约定）。**修复方案不修订本文档主体**，统一在新方案 [hardening-iteration-v1.md](hardening-iteration-v1.md) 的 P0+P1 阶段落实，包括：P0.1 接入 CSRFProtect / P0.2 补 admin_required / P0.3 alerts+portfolio owner 强制 + `alert_history.user_id` 写入端补 / P1.3 `_user_id() raise` 改强制 / P1.5 invariants 加 `alert_history.user_id IS NOT NULL` + `daily_snapshots.user_id IS NOT NULL` 不变式。**根因反思**：本方案 v1.0 §12 实施计划未列出"端到端通路测试"环节（CSRF/admin_required 单元装上但全路由 audit 未做）→ 下版多租户类方案必须在 DoD 明确写"全 POST/DELETE 路由 100% 走 CSRF 中间件 + 全 admin 端点 100% 走 @admin_required" 双 100% 闭环测。 |
