...
// paper-trade v1.4: same banner shape as /analysis/<id> so the page no
// longer renders raw trade_decision markdown. signalLabel/signalVariant
// already exported from AnalysisPage; ConfidenceMeter shipped with
// analysis-rendering v1.0+.
import {
  signalLabel,
  signalVariant,
...
// _rendering_summary_for_analysis (web/app.py). Both the active plan
// and each plan_history row carry one when the linked analysis has a
// rendering_json column; ``latest_analysis_summary`` is the page-level
// fallback used when no plan exists yet.
...
interface ActivePlan {
  ...
  // v1.4: ``trade_decision`` no longer surfaced (raw markdown removed
  // from API). ``analysis_summary`` powers the new banner.
  analysis_summary?: AnalysisSummary | null
  // Legacy holding-months single field for older fixtures.
...
interface PaperPayload {
  ...
  events: Event[]
  dailies: Daily[]
  // v1.4: ``latest_trade_decision`` retained as null for back-compat
  // only; new clients consume ``latest_analysis_summary``.
...
interface Plan {
  ...
  // v1.4: rendering-derived banner DTO. Always present in the v1.4+
  // API; rows whose linked analysis lacks rendering_json get null.
  analysis_summary?: AnalysisSummary | null
}
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        {/* paper-trade v1.4: ``当前策略`` now mirrors the
            /analysis/<id> OverviewCard banner — Rating + tri-state
            signal + ConfidenceMeter + executive_summary — instead of
            the regex-parsed thesis + parse_method footer. */}
        <ActiveStrategyCard
      ...
      {/* paper-trade v1.4: "AI 最终决策" card removed — its content
          ...
          AnalysisHistoryList below both link there. Surfacing the
          same wall-of-text here was a duplicate that bypassed the
          structured 8-tab card system. */}
...
/* ── paper-trade v1.4 — banner card mirroring /analysis/<id> ───────── */

/**
 * Renders the OverviewCard-style banner that the AnalysisDetail page
 * shows: tri-state signal Badge + Rating + ConfidenceMeter + the
 * action_direction / executive_summary lines, with a "查看完整分析 →"
 * link back to ``/analysis/<id>``.
 *
 * Falls back to ``latest_analysis_summary`` (page-level) when
 * ``active_plan`` is null. Returns nothing when both are missing — we
 * never render an empty stub card on a brand-new ticker.
 */
...
/**
 * Replaces the old ``PlanHistory`` in the "执行记录·按 Plan" tab. Each
 * row is a clickable link that opens the linked ``/analysis/<id>``;
 * the topmost row is flagged as ``★ 当前``. Rows whose linked analysis
 * has no rendering_json show a degraded "(结构化数据缺失)" hint
