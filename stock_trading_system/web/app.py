...
# ── Confidence extraction (analysis-rendering v1.7) ──────────────────
#
# Pre-v1.7 the AI-analysis detail page top-of-page confidence number
# came from ``user_analysis_advice.confidence``, which is generated by
# ``strategy_engine._assess_confidence`` — a heuristic that reads the
# trade action (BUY/SELL/HOLD) and produces medium/medium/low. That's
# semantically the *execution* confidence (tied to one user's holdings
# snapshot), not the AI analysis confidence.
#
# v1.7 splits the two: the AI-analysis confidence on the shared row
# now comes from the LLM's structured output, which we already store
# in ``rendering_json``. The OverviewCard's ``confidence`` field is
# the canonical signal; DecisionCard's ``conviction`` is the fallback
# for rows whose Overview extraction failed but Decision succeeded.
#
# We deliberately do NOT fall back to ``advice.confidence`` — that
# would mix the two semantics again. Rows without rendering_json
# expose ``confidence: null`` so the frontend can show "置信度暂无".

_LLM_CONFIDENCE_LEVELS = {"high": 0.85, "medium": 0.5, "low": 0.25}


def _llm_confidence_from_rendering(
    rendering: dict | None,
) -> tuple[str | None, float | None]:
    """Read the LLM-derived analysis confidence out of an already-parsed
    ``rendering`` dict.

    Returns ``(level, numeric)``:
        * ``level`` ∈ {"high", "medium", "low"} or ``None`` when neither
          OverviewCard.confidence nor DecisionCard.conviction is present.
        * ``numeric`` is the gauge-friendly 0.85 / 0.5 / 0.25 mapping.

    The caller is expected to pass the same ``rendering`` it returns
    in the DTO so we don't double-decode the JSON blob.
    """
    ...
    if isinstance(summary, dict):
        ...
        if isinstance(raw, str):
            ...
            if candidate in _LLM_CONFIDENCE_LEVELS:
                level = candidate

    if level is None:
        decision = rendering.get("Decision")
        if isinstance(decision, dict):
            raw = decision.get("conviction")
            if isinstance(raw, str):
                candidate = raw.strip().lower()
                if candidate in _LLM_CONFIDENCE_LEVELS:
                    level = candidate
    ...
    if level is None:
        return None, None
    return level, _LLM_CONFIDENCE_LEVELS[level]


def _llm_confidence_from_record(
    record: dict | None,
) -> tuple[str | None, float | None]:
    """Convenience wrapper for callers (like the list endpoint) that
    have a raw analysis_history row but no parsed rendering yet."""
    if not isinstance(record, dict):
        return None, None
    rendering = _parse_rendering(record.get("rendering_json"))
    return _llm_confidence_from_rendering(rendering)
...
def create_app(config_path=None):
    ...
    def api_analysis_history():
        ...
        completed_items = []
        for rec in records:
            # analysis-rendering v1.7: surface LLM-derived confidence on
            # the inbox row so the front-end can show a "高/中/低置信"
            ...
            completed_items.append({
                ...
                "confidence_source": (
                    "llm_structured_output" if llm_level is not None else None
                ),
            })
    ...
    def api_analysis_detail(analysis_id):
        ...
        # analysis-rendering v1.7: top-of-page confidence is the LLM
        # structured output — ``rendering.summary.confidence`` (preferred)
        # or ``rendering.Decision.conviction`` (fallback). We do NOT
        # peek at advice.confidence: that's the per-user execution
        # confidence (StrategyEngine heuristic) and doesn't represent
        # the shared analysis. Rows without rendering_json get null.
        rendering = _parse_rendering(record.get("rendering_json"))
        confidence_level, confidence_num = _llm_confidence_from_rendering(rendering)
        # ``risk_level`` (a separate user-facing column) historically
        # mirrored the legacy advice-confidence string when no risk
        # field was set. Keep that visual fallback so the "风险等级"
        # tile doesn't suddenly read "—" for old rows; this is a
        # cosmetic compatibility aid, not the analysis confidence.
        legacy_advice_conf_str = (
            advice.get("confidence") or ""
        ).lower() if isinstance(advice.get("confidence"), str) else ""
        ...
        return jsonify({
            ...
            "analysts":           analysts,
            # AI 分析置信度（v1.7+）：来自 LLM 结构化输出而非个人 advice
            # 启发式。``confidence`` 仍是 0.85/0.5/0.25/null 数值，前端
            # 仪表保持兼容；``confidence_level`` 暴露原始 high/medium/low
            # 字符串，前端可渲染中文标签；``confidence_source`` 为常量
            # ``"llm_structured_output"`` 当前；rendering 缺失时 null
            # 而**非** fallback 到 advice.confidence。
            "confidence":         confidence_num,
            "confidence_level":   confidence_level,
            "confidence_source":  (
                "llm_structured_output" if confidence_level is not None else None
            ),
            "risk_level":         (
                advice.get("risk_level") or legacy_advice_conf_str or "-"
            ),
            ...
            # expecting structured data. Same ``rendering`` instance the
            # v1.7 confidence helper read above — single decode per call.
            "rendering":          rendering,
        })
    ...
    def api_settings():
        ...
        # 2026-05-04: surface which fields are env-var-locked so the
        # Settings UI can show a hint instead of silently letting the
        # user save a value that gets stomped at next boot. Mirror the
        # ``env_map`` in ``settings._apply_env_overrides``.
        env_locked = {
            "gemini.api_key":     bool(os.environ.get("GEMINI_API_KEY")),
            "gemini.model":       bool(os.environ.get("GEMINI_MODEL")),
            "qwen.api_key":       bool(os.environ.get("DASHSCOPE_API_KEY")
                                        or os.environ.get("QWEN_API_KEY")),
            "qwen.model":         bool(os.environ.get("QWEN_MODEL")),
            "polygon.api_key":    bool(os.environ.get("POLYGON_API_KEY")),
            "ib.host":            bool(os.environ.get("IB_HOST")),
            "ib.port":            bool(os.environ.get("IB_PORT")),
            "alerts.telegram.bot_token": bool(os.environ.get("TELEGRAM_BOT_TOKEN")),
            "alerts.telegram.chat_id":   bool(os.environ.get("TELEGRAM_CHAT_ID")),
            "alerts.email.smtp_host":    bool(os.environ.get("EMAIL_SMTP_HOST")),
            "alerts.email.username":     bool(os.environ.get("EMAIL_USERNAME")),
            "alerts.email.password":     bool(os.environ.get("EMAIL_PASSWORD")),
            "alerts.email.to_address":   bool(os.environ.get("EMAIL_TO")),
        }
        ...
        return jsonify({
            ...
            # Map of dotted-path → True when a process env var pins that
            # field. UI uses it to show "锁定，env 控制" hint and skip
            # the value in the POST payload (frontend handles both).
            "env_locked": {k: v for k, v in env_locked.items() if v},
        })
    ...
    def api_paper_ticker_detail(ticker: str):
        ...
        # ActiveStrategyCard renders the same banner /analysis/<id>
        ...
        # so the front-end "执行记录·按 Plan" tab renders a clickable
        # AI-analysis history list. Raw ``trade_decision`` markdown is
        # NO LONGER returned — it's already on /analysis/<id>, and the
        # paper-trade page links there rather than rendering a copy.
        all_plans: list[dict] = []
        for s_id in sib_ids:
        ...
        # compat; new clients consume ``latest_analysis_summary`` which
